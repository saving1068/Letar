<template>
	<section class="seekPage cf">
		...
		<div class="seekPageLeft">
			<h2>
				<span @click="goToseekPageAll">全部课程<i class="fa fa-angle-right"></i></span>
				<span v-if="this.$route.query.firstName">{{this.$route.query.firstName}}<i class="fa fa-angle-right"></i></span>
			</h2>
			...
			<div class="curriculumNav cf">
				...
				<div class="curriculumTabNav">
					<ul class="curriculumTabNavA cf" id="navA">
						<li v-for="(item,index) in navA" v-text="item.name" @click="navA_click(index,item.a,item.name)"></li>
						<!--<li>VIP课程</li>
						<li>付费专栏</li>
						<li>免费课程</li>
						<li>互动直播</li>-->
...
export default {
  ...
  created(){
		this.getCouseList(0,2,this.$route.query.firstClass,24)
		this.getHotList(0,1)
  },
  watch:{
  	classType:(val)=>{
  ...
  methods:{
  	...
  	current_change(val){
  		...
  		this.ifNav = false;
  		this.classPage = val
  		this.getCouseList(this.classType,2,this.$route.query.firstClass,24,1,val)
  	},
  	next(){
  		this.ifNav = false;
  		this.classPage++
  		this.getCouseList(this.classType,2,this.$route.query.firstClass,24,1,this.classPage)
  	},
  	prev(){
  		this.ifNav = false;
  		this.classPage--
  	...
  	getPriceSort(){
  		...
  		this.ifNav = false;
  		if(this.priceSort){
  			this.getCouseList(this.classType,2,this.$route.query.firstClass,24,1)
  		}else{
  			this.getCouseList(this.classType,2,this.$route.query.firstClass,24,2)
  		}
  	},
  	goToDetile(courseId){
  	...
  	nav_click(index,id,thirdName){
		...
		this.$router.push({
			...
			query:{
				...
				secondClass:id,
				secondName:this.$route.query.name,
				thirdName,
			}
		})
...
//		if(this.classType !=null){}
		...
		this.getCouseList(a,2,this.$route.query.firstClass,24)
