<template>
	...
	<section class="agencyA">
		...
		<div class="agencyAContent">
			...
			<form class="cf">
				<p><span>*</span>资格证书选择</p>
				<select class="missCard">
					<option value="教师资格证">教师资格证</option>
					<option value="最高学历证">最高学历证</option>
				</select>
			</form>
			<form class="cf">
				<p><span>*</span>资格证书名称</p>
				<input type="text" placeholder="请填写证书名称" required="required" id="status" @blur="statusVerify" maxlength="16"/>
				<span class="verify"></span>
			</form>
			<form class="cf fileBox fileMissCard">
				<p><span>*</span>上传资格证</p>

				<div class="fileItem fileMaginR">
					<div class="fileBg">
						<i class="el-icon-plus"></i>
						<span>点击上传图片</span>
						<input class="file" type="file" accept="image/*" name="file[]" id="publish-imgC"  @change="uploadImgC" ref="imgInputC"/>
						<img :src="userURLC" v-if="isUrlC"/>
					</div>
				</div>
				
				<ol class="fileItemText">
					<li>可选择：教师资格证、高学历证</li>
					<li>等；请提交有效期内的证件照</li>
					<li>片，需确保头像和文字清晰，</li>
					<li>文件小于2M</li>
				</ol>
			...
			<form class="cf">
				<p><span>*</span>推荐人手机</p>
				<input type="text" placeholder="请填写手机号码" required="required" id="recommendMob" @blur="recommendMobVerify" maxlength="11"/>
				<span class="verify"></span>
			</form>
			<form class="cf">
				<p><span>*</span>手机号码</p>
			...
			<form>
				<input type="text" placeholder="验证码" class="code" maxlength="6" :disabled="disInput" v-model="code"/>
  ...
  methods:{
	...
	uploadImgC(e) { //自定义的上传图片；
		...
		this.$Api.UploadImage(config).then((res) => { //上传图片返回的接口信息；
			console.log(res)
			if(res.q.s == 0) {
				this.user.imgIdC = res.q.files[0].id
			}
  	...
  	submitData(){ //点击提交资料跳转到入驻代理商——审核页面；
		let agencyAContent = document.querySelector('.agencyAContent'),
			...
			agencyAContentFormInput1 = agencyAContentFormList[0].querySelector('input'),  //真实姓名输入框；
			agencyAContentFormInput2 = agencyAContentFormList[2].querySelector('input'),  //证件号码输入框；
		    agencyAContentFormInput3 = agencyAContentFormList[5].querySelector('input'),  //资格证书名称输入框；
		    agencyAContentFormInput4 = agencyAContentFormList[8].querySelector('input'),  //我的职业输入框；
		    agencyAContentFormInput5 = agencyAContentFormList[9].querySelector('input'),  //邮箱输入框； 
		    agencyAContentFormInput6 = agencyAContentFormList[11].querySelector('input'),  //手机号码输入框；
		    agencyAContentFormInput7 = agencyAContentFormList[10].querySelector('input'),   //推荐人手机输入框；
		    agencyAContentFormInput8 = agencyAContentFormList[12].querySelector('input'),   //验证码输入框；
		    sex = document.querySelector('#sex'), //性别；
		    idCard = document.querySelector('#idCard'), //身份证明；
		    missCard = document.querySelector('.missCard'); //资格证书选择；
		
		let nameVerify = /^[\u4E00-\u9FA5A-Za-z]+$/, //真实姓名正则；
			IDnumberVerify = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/, //证件号码正则；
			statusVerify = /^[\u4E00-\u9FA5A-Za-z]+$/, //资格证书名称；
		...
		if (nameVerify.test( agencyAContentFormInput1.value.trim() )&&
			IDnumberVerify.test( agencyAContentFormInput2.value.trim() )&&
			statusVerify.test( agencyAContentFormInput3.value.trim() )&&
			professionVerify.test( agencyAContentFormInput4.value.trim() )&&
			...
			agencyAContentFormInput8.value.trim()) {
				...
				this.$Api.getSmsCode({'a':2,'type':9,'mobile':agencyAContentFormInput6.value,'code':agencyAContentFormInput8.value}).then((res) =>{  //验证验证码；
					...
					if (res.q.s != 0) {
						...
						this.$Api.getTeacherRegister({	'a':1,
														'name':agencyAContentFormInput1.value,  //真实姓名；
														'idNum':agencyAContentFormInput2.value,  //证件号码；
														'idImages':this.imgIdA +","+ this.imgIdB,  //上传两张身份证照片的 id;
														'quaType':idCard.value,  //资格证书类型;
														'quaImage':this.imgIdC,  //附带案例通过;
														'sex':sex.value,  //性别；
														'professional':agencyAContentFormInput4.value,  //职业；
														'email':agencyAContentFormInput5.value,  //邮箱；
														'mobile':agencyAContentFormInput6.value, //手机；
														'recommendMobile':agencyAContentFormInput7.value,  //推荐人手机；
														'smsKey':this.smsKey  ////验证码返回；
						}).then((res)=>{  //老师入驻申请；
							console.log(res)
  	...
  	IDnumberVerify(){ //证件号码验证；
  		...
  		if (!IDnumber.value.trim()) {  //输入框为空时；
			verifys[1].innerHTML = '证件号码不能为空';
			verifys[1].style.color = 'red';
		}else{
			if(IDnumberVerify.test(IDnumber.value)){//如果验证正确执行以下代码
				verifys[1].innerHTML = '';
			}else {//验证不成功，执行以下代码
				verifys[1].innerHTML = '证件号码格式不正确，请重新输入';
				verifys[1].style.color = 'red';
			}
		}
  		
  	},
  	statusVerify(){ //资格证书名称验证；
  		var status = document.querySelector('#status'),
  		    verifys = document.querySelectorAll('.verify'), 		
  		    statusVerify = /^[\u4E00-\u9FA5A-Za-z]+$/; //资格证书名称；
  		
  		if (!status.value.trim()) {  //输入框为空时；
			verifys[2].innerHTML = '资格证书名称不能为空';
			verifys[2].style.color = 'red';
		}else{
			if(statusVerify.test(status.value)){//如果验证正确执行以下代码
				verifys[2].innerHTML = '';
			}else {//验证不成功，执行以下代码
				verifys[2].innerHTML = '不能有数字，请重新输入';
				verifys[2].style.color = 'red';
			}
		}
...
.agencyA{
	...
	.agencyAContent{
		...
		form{
			...
			.fileMaginR{margin-right: 11px !important;}
			...
			.missCard{width: 158px;}
		}
		.fileBox{height: 139px;}
		.fileMissCard{height: 99px;}
...
.fileItemText{
	width: 169px;height: 100%;float: right;color: #808080;
