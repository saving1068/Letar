<template>
	<div>
		<div id="second_warp">
	    	...
	    	<div class="free" v-for="(v,i) in allArr">
	    		<div class="free_top">
	    			<p>{{v.name}}</p>
	    		...
	    		<div class="vipClass_content">
					<div class="vipClass_content_tab">
						...
						<ul class="vipClass_content_tab_warp" v-if="v.id == allArr[0].id">
							<li class="vipClass_content_tab_item" :class="{'active':vipTwo_1_num == i}" v-for="(v,i) in v.course_category" @click="chang(i,0)">{{v.name}}
								
							</li>
						</ul>
						<ul class="vipClass_content_tab_warp" v-if="v.id == allArr[1].id">
							<li class="vipClass_content_tab_item" :class="{'active':vipTwo_2_num == i}" v-for="(v,i) in v.course_category" @click="chang(i,1)">{{v.name}}
								
							</li>
						</ul>
						<ul class="vipClass_content_tab_warp" v-if="v.id == allArr[2].id">
							<li class="vipClass_content_tab_item" :class="{'active':vipTwo_3_num == i}" v-for="(v,i) in v.course_category" @click="chang(i,2)">{{v.name}}
								
							</li>
						</ul>
						<ul class="vipClass_content_tab_warp" v-if="v.id == allArr[3].id">
							<li class="vipClass_content_tab_item" :class="{'active':vipTwo_4_num == i}" v-for="(v,i) in v.course_category" @click="chang(i,3)">{{v.name}}
								
							</li>
						</ul>
					</div>							
					<vcontent v-if="v.id == allArr[0].id" :message="one"></vcontent>
					<vcontent v-if="v.id == allArr[1].id":message="two"></vcontent>
					<vcontent v-if="v.id == allArr[2].id" :message="three"></vcontent>
					<vcontent v-if="v.id == allArr[3].id" :message="four"></vcontent>
	    		</div>
	    	</div>
    	</div>
	</div>
...
//			if(this._props.classId == 2){//VIP
				this.getSelected()
				this.getClassId(this._props.classId)
				
//				this.getClassList(1,0)
//				this.getClassList(2,0)
//				this.getClassList(3,0)
//			}
...
//				console.log(index)
			...
			chang(i,type){
				if(type == 0){
					if(i||i==0){
						this.vipTwo_1_num = i
						this.getClassList(this.allArr[0].id,type,i)
					}
				}else if(type == 1){
					if(i||i==0){
						this.vipTwo_2_num = i
						this.getClassList(this.allArr[1].id,type,i)
					}
				}else if(type == 2){
					if(i||i==0){
						this.vipTwo_3_num = i
						this.getClassList(this.allArr[2].id,type,i)
					}
				}else if(type == 3){
					if(i||i==0){
						this.vipTwo_4_num = i
						this.getClassList(this.allArr[3].id,type,i)
					}
				}
				
			},
			getClassList(firstClass,i,y){
				console.log(firstClass,this.allArr[i].course_category[y].id,"ID")
				if(this.$store.state.isLogin){
					var obj ={
						a: this._props.classId,
						firstClass: firstClass,
						secondClass: this.allArr[i].course_category[y].id
					}
				}else{
					var obj ={
						a: this._props.classId,
				...
				this.$Api.getCourseList(obj).then((res)=>{
					console.log(res,"第三方V地方·")
					if(firstClass == this.allArr[0].id){
						this.one = res.q.course
					}else if(firstClass == this.allArr[1].id){
						this.two = res.q.course
					}else if(firstClass == this.allArr[2].id){
						this.three = res.q.course
					}else if(firstClass == this.allArr[3].id){
						this.four = res.q.course
					}
				})
			},
...
//					console.log(this.onlineArr,"精选推送")
			...
			getClassId(classType){
				var that = this;
				this.$Api.PcCouseList({a:classType,li:6}).then((res)=>{
//					console.log(res,"cxzcasdasdasd")
					this.VipLiveArr = res.q.course;
					this.allArr = res.q.categorys;
					var num = 0;
					if(this.allArr.length == 4){
						this.getClassList(this.allArr[0].id,0,0)
						this.getClassList(this.allArr[1].id,1,0)
						this.getClassList(this.allArr[2].id,2,0)
						this.getClassList(this.allArr[3].id,3,0)
					}else{
						var that = this;
						var tierm = setInterval(()=>{
							num++;
							if(num ==  that.allArr.length){
								clearInterval(tierm)
							}
							this.getClassList(this.allArr[num].id,num,0)
						},1000)
					}
...
<style scoped="scoped">
	...
	.free_more{display: flex;font-size: 18px;position: absolute;top: 34px;right: 0;width: 50px;justify-content: space-between;cursor: pointer;}
	...
	.vipClass_content_tab{padding:20px 0;overflow: hidden;font-size: 20px;color: #808080;width: 1300px;margin: 0 auto;}
